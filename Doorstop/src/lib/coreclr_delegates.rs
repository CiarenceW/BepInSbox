...
type LoadAssemblyAndGetFunctionPointerArgs = 
	(
        ...
        *mut ::std::ffi::c_void,      /* reserved Extensibility parameter (currently unused and must be 0). */
        *mut *mut ::std::ffi::c_void, /* delegate Pointer where to store the function pointer result). */
    );
...
pub type GetFunctionPointerFn = 
	unsafe extern "C" fn(
        ...
        delegate_type_name: *const char_t,          /* Assembly qualified delegate type name or null,
                                        			   or UNMANAGEDCALLERSONLY_METHOD if the method is marked with
                                        			   the UnmanagedCallersOnlyAttribute. */
        load_context: *mut ::std::ffi::c_void,  /* Extensibility parameter (currently unused and must be 0) */
        reserved: *mut ::std::ffi::c_void,      /* Extensibility parameter (currently unused and must be 0) */
