...
using BepInSbox.NET.CoreCLR;
using BepInSbox.NET.Shared;
...
internal class StartupHook
{
    ...
    {
        var silentExceptionLog = $"bepins&x_preloader_{DateTime.Now:yyyyMMdd_HHmmss_fff}.log";
        ...
        try
        {
            ...
            string bepinsboxCoreDirectory = null;

            if (gameDirectory != null)
                bepinsboxCoreDirectory = Path.Combine(gameDirectory, "BepInSbox", "core");

            if (executableFilename == null || gameDirectory == null || !Directory.Exists(bepinsboxCoreDirectory))
            {
                throw new Exception("Could not determine game location, or BepInSbox install location");
            }
            
            silentExceptionLog = Path.Combine(gameDirectory, silentExceptionLog);
            
            ResolveDirectories.Add(bepinsboxCoreDirectory);
//#endif
